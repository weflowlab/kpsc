/* ==========================================================================
   ...
   - 약관 동의는 체크박스가 아니라 라디오 2쌍이며, 기본값은 "동의합니다"
     (클라이언트 요청). 약관 전문은 관리자(환경설정)에서 편집한 DB 값을
     서버 페이지(app/register/page.tsx)가 넘겨준다
   ========================================================================== */
...
import { checkLoginId, register } from "@/app/actions/auth";
...
export default function RegisterForm({
  ...
  /** 홈페이지 이용약관 전문 (관리자 편집본) */
  termsText: string;
  /** 개인정보 처리방침 전문 (관리자 편집본) */
  privacyText: string;
}) {
  /* 입력 상태 */
  ...
  /* 아이디 중복확인 상태 — 확인한 아이디 값을 기억해 두고,
     아이디가 바뀌면 다시 확인하도록 강제한다 */
  const [checkedId, setCheckedId] = useState<string | null>(null);
  const [idNotice, setIdNotice] = useState<string | null>(null);
  ...
  const onCheckId = async () => {
    const res = await checkLoginId(form.MB_ID);
    setIdNotice(res.message);
    setCheckedId(res.available ? form.MB_ID : null);
  };

  /* ------------------------------------------------------------------
     제출 — 원본 알럿 문구로 클라이언트 검증 후 서버 액션 호출
     ------------------------------------------------------------------ */
  const onSubmit = async (e: FormEvent) => {
    ...
    if (checkedId !== form.MB_ID)
      return setError("아이디 중복확인을 해주세요.");
    ...
    try {
      const res = await register({
        name: form.MB_NAME,
        loginId: form.MB_ID,
        password: form.MB_PW1,
        email: form.MB_EMAIL,
        phone1: form.MB_HAND_TEL1,
        phone2: form.MB_HAND_TEL2,
        phone3: form.MB_HAND_TEL3,
        remail,
      });
      if (!res.ok) {
        setError(res.error);
      ...
      /* 가입과 동시에 자동 로그인 — 헤더 상태까지 반영되도록 전체 로드 */
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                value={form.MB_ID}
                onChange={(e) => {
                  set("MB_ID")(e.target.value);
                  /* 아이디를 고치면 중복확인을 다시 하도록 초기화 */
                  setCheckedId(null);
                  setIdNotice(null);
                }}
                className={inputClass}
            ...
            {/* 중복확인 결과 — 사용 가능하면 초록, 아니면 경고색 */}
            {idNotice && (
              <p
                className={`mt-1 text-[12px] ${
                  checkedId ? "text-green-600" : "text-board-tag"
                }`}
              >
                {idNotice}
              </p>
        ...
        >
          ...
          <div className="relative flex max-h-[80vh] w-full max-w-[560px] flex-col rounded-lg bg-white">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 text-[13px] leading-[1.9] whitespace-pre-line text-ink-500">
              {popup === "privacy" ? privacyText : termsText}
            </div>
